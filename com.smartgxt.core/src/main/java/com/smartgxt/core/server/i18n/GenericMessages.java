...
public class GenericMessages extends GenericX {
	...
	public GenericMessages(Class<?> _itf, String lang) throws IOException,
	...
	private String buildMessage(String propertyName, Method method,
		...
		PluralText pluralTextAnnotation = method
		...
		for (int i = 0; i < paramTypes.length; i++) {
			...
			PluralCount pc = getAnnotation(paramsAnnotations[i],
					PluralCount.class);
			if (pc == null) {
				continue;
				...
				int n = -1;
				if (isA(paramType, Number.class) || isA(paramType, byte.class)
		...
		if (args != null) {
			for (int i = 0; i < args.length; i++) {
				...
				if (replacedPattern != null) {
					template = template.replaceAll(replacedPattern,
							value == null ? "" : value);
				}
			}
		}
