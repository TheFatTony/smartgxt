 ...
 * 
 */
public class MenuBarLayout extends Layout {
	...
	protected void addComponentToMenu(Menu menu, Component c) {
		if (c instanceof MenuBarItem) {
			MenuBarItem menuBarItem = (MenuBarItem) c;
			MenuItem item = new MenuItem(menuBarItem.getText());
			item.setSubMenu(menuBarItem.getMenu());
			menu.add(item);
		}
	}
	...
	protected void fitToSize(El t) {
		if (!((MenuBar) container).isEnableOverflow()) {
			return;
		}
			...
			moreMenu.addListener(Events.BeforeShow, new Listener<MenuEvent>() {
				...
				public void handleEvent(MenuEvent be) {
					...
					if (be.getContainer().getItemCount() == 0) {
						be.getContainer().add(
								new HeaderMenuItem(noItemsMenuText));
					}
				}
	...
	protected void onComponentHide(Component component) {
		...
		if (component.isRendered()) {
			component.el().getParent()
					.addStyleName(component.getHideMode().value());
		}
	...
	protected void onComponentShow(Component component) {
		...
		if (component.isRendered()) {
			component.el().getParent()
					.removeStyleName(component.getHideMode().value());
		}
	}
