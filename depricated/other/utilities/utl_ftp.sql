--------------------------------------------------------
--  DDL for Package UTL_FTP
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE "ISIN"."UTL_FTP" 
AS
    -- --------------------------------------------------------------------------
    ...
    /*  TYPE t_file_type IS RECORD (TYPE CHAR (1) -- F - file, D - directory
                                               , name VARCHAR2 (4000)
                                               ,last_change VARCHAR2 (100)
                                               ,size_bytes VARCHAR2 (500));*/
    TYPE t_file_type IS RECORD
    (
        attributes_        VARCHAR2 (100)
      , flag_              VARCHAR2 (100)
      , owner_             VARCHAR2 (100)
      , group_             VARCHAR2 (100)
      , size_              VARCHAR2 (100)
      , date_              VARCHAR2 (100)
      , name_              VARCHAR2 (4000)
      ,type_ VARCHAR2 (1) -- F - file, D - directory
    );

    TYPE t_list_files IS TABLE OF t_file_type;

    FUNCTION login (p_host IN VARCHAR2, p_port IN VARCHAR2, p_user IN VARCHAR2, p_pass IN VARCHAR2, p_timeout IN NUMBER := NULL)
    ...
    FUNCTION get_remote_ascii_data (p_conn IN OUT NOCOPY UTL_TCP.connection, p_file IN VARCHAR2)
        RETURN CLOB;

    FUNCTION get_remote_binary_data (p_conn IN OUT NOCOPY UTL_TCP.connection, p_file IN VARCHAR2)
        RETURN BLOB;
    ...
    PROCEDURE list (p_conn IN OUT NOCOPY UTL_TCP.connection, p_dir IN VARCHAR2, p_list OUT t_list_files);
    ...
    --   PROCEDURE delete (p_conn IN OUT NOCOPY UTL_TCP.connection, p_file IN VARCHAR2, p_replay IN BOOLEAN);
...
/
--------------------------------------------------------
--  DDL for Package Body UTL_FTP
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY "ISIN"."UTL_FTP" 
AS
    -- --------------------------------------------------------------------------
    ...
    BEGIN
        ...
        l_port1     :=
            TO_NUMBER (SUBSTR (l_reply, INSTR (l_reply, '.', 1, 4) + 1, (INSTR (l_reply, '.', 1, 5) - 1) - (INSTR (l_reply, '.', 1, 4))));
    ...
    BEGIN
        LOOP
            ...
            ELSIF (SUBSTR (g_reply (g_reply.LAST), 1, 3) = l_reply_code AND SUBSTR (g_reply (g_reply.LAST), 4, 1) = ' ')
            THEN
                EXIT;
            END IF;
        END LOOP;
    ...
    BEGIN
        IF g_binary
        THEN
            put_local_binary_data (p_data => get_remote_binary_data (p_conn, p_from_file), p_dir => p_to_dir, p_file => p_to_file);
        ELSE
            put_local_ascii_data (p_data => get_remote_ascii_data (p_conn, p_from_file), p_dir => p_to_dir, p_file => p_to_file);
        END IF;
    END;
    ...
    BEGIN
        IF g_binary
        THEN
            put_remote_binary_data (p_conn => p_conn, p_file => p_to_file, p_data => get_local_binary_data (p_from_dir, p_from_file));
        ELSE
            put_remote_ascii_data (p_conn => p_conn, p_file => p_to_file, p_data => get_local_ascii_data (p_from_dir, p_from_file));
        END IF;
    ...
    PROCEDURE list (p_conn IN OUT NOCOPY UTL_TCP.connection, p_dir IN VARCHAR2, p_list OUT t_list_files)
    AS
        o_file_list          utl_ftp.t_string_table;
        o_definitions        utl_ftp.t_list_files := utl_ftp.t_list_files ();
        v_string             VARCHAR2 (32000);
        n_start              PLS_INTEGER;
        n_end                PLS_INTEGER;
        n_diff PLS_INTEGER;
    BEGIN
        list (p_conn => p_conn, p_dir => p_dir, p_list => o_file_list);

        IF o_file_list.LAST > 0
        THEN
            FOR i IN o_file_list.FIRST .. o_file_list.LAST
            LOOP
                o_definitions.EXTEND ();

                v_string    := REPLACE (REPLACE (REPLACE (o_file_list (i), '  ', ' ~'), '~ '), '~');

                n_diff := LENGTH(o_file_list (i)) - LENGTH(v_string);

                n_start     := INSTR (v_string, ' ', 1, 1);
                n_end       := INSTR (v_string, ' ', 1, 2);
                o_definitions (i).attributes_ := SUBSTR (v_string, 1, n_start - 1);
                o_definitions (i).flag_ := SUBSTR (v_string, n_start + 1, n_end - n_start - 1);
                n_start     := n_end;
                n_end       := INSTR (v_string, ' ', 1, 3);
                o_definitions (i).owner_ := SUBSTR (v_string, n_start + 1, n_end - n_start - 1);
                n_start     := n_end;
                n_end       := INSTR (v_string, ' ', 1, 4);
                o_definitions (i).group_ := SUBSTR (v_string, n_start + 1, n_end - n_start - 1);
                n_start     := n_end;
                n_end       := INSTR (v_string, ' ', 1, 5);
                o_definitions (i).size_ := SUBSTR (v_string, n_start + 1, n_end - n_start - 1);
                n_start     := n_end;
                n_end       := INSTR (v_string, ' ', 1, 8);
                o_definitions (i).date_ := SUBSTR (v_string, n_start + 1, n_end - n_start - 1);

                o_definitions (i).name_ :=
                    SUBSTR (o_file_list (i), INSTR (v_string, o_definitions (i).date_) + n_diff + LENGTH (o_definitions (i).date_) + 1);

                o_definitions (i).type_ := REPLACE (SUBSTR (o_definitions (i).attributes_, 1, 1), '-', 'F');
            END LOOP;

            p_list      := o_definitions;
        ELSE
            p_list      := NULL;
        END IF;
    END list;
    ...
    BEGIN
        l_conn   := get_passive (p_conn);
        send_command (p_conn, 'DELE ' || p_file, p_replay);
--        LOGOUT (l_conn, FALSE);
    END delete;*/
